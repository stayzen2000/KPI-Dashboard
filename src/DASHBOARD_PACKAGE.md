# AI Agency KPI Dashboard - Complete Package

## Overview
A dark-themed, single-page KPI dashboard built with React, TypeScript, and Tailwind CSS for tracking AI agency metrics.
...
### 2. Component Files
All component files are provided in the sections below.

### 3. Styles
The globals.css file is provided below.

## Customization

### Updating Data
Each chart component contains mock data at the top of the file. Replace these with your actual data:

- `DialCountChart.tsx` - Line chart data for dial counts
- `ProspectsDialedChart.tsx` - Bar chart data for prospects dialed
- `ProspectQualityPieChart.tsx` - Pie chart data for prospect quality distribution
- `ProspectQualityByDayChart.tsx` - Stacked bar chart data for daily quality metrics
- `KPICard.tsx` - Individual KPI metrics in the sidebar

### Color Scheme
The dashboard uses a dark theme with the following primary colors:
- Background: `#0a0e27`
...
- Accent Colors:
  - Cyan: `#06b6d4`
  - Yellow: `#fbbf24`
  - Green (Interested): `#84cc16`
  - Red (Not Interested): `#ef4444`
  - Orange (Follow-Up): `#f59e0b`
  - Gray (Dead): `#6b7280`

### Responsive Design
The dashboard is optimized for desktop viewing at 1920x1080 and above. For smaller screens, you may need to adjust the grid layout in `App.tsx`.
...
### /src/components/DialCountChart.tsx
```typescript
...
### /src/components/ProspectsDialedChart.tsx
```typescript
...
### /src/components/MonthGoalCard.tsx
```typescript
...
### /src/components/ProspectsCard.tsx
```typescript
...
### /src/components/ProspectQualityPieChart.tsx
```typescript
...
### /src/components/ProspectQualityByDayChart.tsx
```typescript
...
export function ProspectQualityByDayChart() {
  return (
    <div className="bg-[#0f1535] p-3 rounded-lg border border-[#1a2150] h-full flex flex-col">
      <h3 className="text-white mb-2 text-sm">Prospect Quality By Day</h3>
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1a2150" />
            ...
            <Legend 
              ...
              iconType="circle"
            />
            <Bar dataKey="Interested" stackId="a" fill="#84cc16" radius={[0, 0, 0, 0]} />
            <Bar dataKey="Not Interested" stackId="a" fill="#ef4444" radius={[0, 0, 0, 0]} />
            <Bar dataKey="Follow-Up" stackId="a" fill="#f59e0b" radius={[0, 0, 0, 0]} />
            <Bar dataKey="VM/Ringing" stackId="a" fill="#06b6d4" radius={[0, 0, 0, 0]} />
            <Bar dataKey="Dead" stackId="a" fill="#6b7280" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
...
- The dashboard is designed to fit on one screen without scrolling
- Charts are responsive and will adjust to their container sizes
- Mock data is included in each component for demonstration purposes
- Colors are hardcoded but can be extracted to a theme configuration file if needed
- The layout uses CSS Grid and Flexbox for responsive positioning

## License

This code is provided as-is for use in your AI agency dashboard project.
